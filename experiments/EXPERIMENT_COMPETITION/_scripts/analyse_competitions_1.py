...
def plot_results_by_case(results_by_case):
    ...
    axs[0].set_xlabel("$\\sigma_{\\text{env}}$")
    axs[0].set_ylabel("Average margin")
    axs[0].legend()
    axs[0].grid()
    axs[0].set_ylim(0.988, 1.012)
    axs[1].set_xlabel("$\\sigma_{\\text{env}}$")
    axs[1].set_ylabel("SD of margin")
    axs[1].legend()
    axs[1].grid()   
    axs[2].set_xlabel("$\\sigma_{\\text{env}}$")
    axs[2].set_ylabel("Average reward")
    axs[2].legend()
    axs[2].grid()
    plt.tight_layout()
    plt.savefig("../competition_analysis_1/summary_all_cases_comp_1.png")
    plt.close()
