...
Specifically, it analyses the competitions between two agents under different noise levels
...
def compute_average_margins(result, c):
    ...
    # Compute average reward per episode and SE of the average reward
    ...
    # Compute correlation fo margins 
    correlations = np.corrcoef(margins.reshape(no_sims*iterations, n_agents).T)
    avg_correlation = correlations[0, 1]
...
def analyse_case(name):
    ...
    output_dir = f"../competition_analysis_2/{name}/"
...
def plot_summary_margins_vs_corr_and_noise(results_by_case):
    ...
    naming_dict = {
        "eps_vs_eps": "SW-$\\varepsilon$-U vs SW-$\\varepsilon$-U",
        "ucb_vs_ucb": "SW-UCB-U vs SW-UCB-U",
        "uts_vs_uts": "SW-UTS vs SW-UTS",
    }
    ...
    for i, (case_name, results) in enumerate(results_by_case.items()):
        ...
        for corr in corrs:
            ...
            # Plot average with shaded SE area
            ax.plot(
                std_values,
                avg_margin_values,
                label=f"$\\rho$={corr}",
        ...
        ax.grid()
    plt.tight_layout()
    plt.savefig("../competition_analysis_2/summary_margins_with_correlations.png")
    # plt.show()
    plt.close()
...
def plot_summary_all_cases(case_results):
    ...
    for i, (case_name, results) in enumerate(case_results.items()):
        ...
        for std in stds:
            if (std, corr) in results:
                ...
                std_margin_values.append(
                    results[(std, corr)]["sd_margins"][0]
                )
                corr_values.append(
                    results[(std, corr)]["avg_correlation"]
                )
        ...
        avg_margin_values = np.array(avg_margin_values)
        se_avg_margin_values = np.array(se_avg_margin_values)
        std_margin_values = np.array(std_margin_values)
        corr_values = np.array(corr_values)
        avg_reward_values = np.array(avg_reward_values)
        se_avg_margin_values = np.array(se_avg_margin_values)
        ...
        axs[1, 1].fill_between(
            std_values,
            np.array(avg_reward_values) - np.array(se_avg_margin_values),
            np.array(avg_reward_values) + np.array(se_avg_margin_values),
            alpha=0.2
        )
    # No titles but axis names
    axs[0, 0].set_ylabel("Average margin")
    axs[0, 1].set_ylabel("Std of margin")
    ...
    for ax in axs.flat:
        ...
        ax.legend()
    plt.tight_layout()
    plt.savefig("../competition_analysis_2/summary_all_cases_comp_2_stat.png")
